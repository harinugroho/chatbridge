...
const patchGetChatModel = async (client) => {
  console.error('[DEBUG-client-check]', { hasClient: !!client, hasPupPage: !!(client && client.pupPage) });
  if (client) {
    console.error('[DEBUG-client-keys]', Object.keys(client));
  }
  if (client && client.pupPage) {
    try {
      const res = await client.pupPage.evaluate(() => {
        if (!window.WWebJS) {
          return 'window.WWebJS is undefined';
        }
        ...
        window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
            ...
            try {
              ...
              if (isChannel) {
                  ...
                  model.isChannel = getters ? getters.getIsNewsletter(chat) : false;
              } else {
              ...
              if (chat.groupMetadata) {
                  ...
                  try {
                      ...
                      console.warn('Failed to update group metadata for', chat.id._serialized, e);
                  }
                  
                  try {
                      const migrationUtils = window.require('WAWebLidMigrationUtils');
                      const toPn = migrationUtils ? migrationUtils.toPn : null;
                      ...
                      console.warn('Failed to serialize group metadata for', chat.id._serialized, e);
                  }
              ...
              if (chat.newsletterMetadata) {
                  try {
                      ...
                      console.warn('Failed to update newsletter metadata for', chat.id._serialized, e);
                  }
                  try {
                      ...
                      console.warn('Failed to serialize newsletter metadata', e);
                  }
              }
              ...
              try {
                  ...
                  console.warn('Failed to get lastMessage for', chat.id._serialized, e);
              }
              ...
              console.error('Failed to getChatModel for chat:', chat?.id?._serialized, err);
              return null;
            }
        };
        ...
        window.WWebJS.getChats = async () => {
            try {
                ...
                if (!collections || !collections.Chat) {
                    console.error('WAWebCollections.Chat is undefined');
                    return [];
                }
                ...
                console.error('Error in window.WWebJS.getChats:', err);
                throw err;
            }
        };

        return 'Patches for getChatModel and getChats injected successfully';
      });
      console.error('[DEBUG-patch-result]', res);
    } catch (err) {
      console.error('Failed to inject getChatModel patch', err);
    }
  }
}
...
const getChats = async (req, res) => {
  ...
  */
  try {
    const client = sessions.get(req.params.sessionId)
    await patchGetChatModel(client)
    const chats = await client.getChats()
    res.json({ success: true, chats })
  } catch (error) {
    console.error('[DEBUG-getChats-error]', error);
    sendErrorResponse(res, 500, error.message)
  }
}
